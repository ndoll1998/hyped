...
class TokenizerProcessorConfig(DataProcessorConfig):
    ...
    Arguments:
        pretrained_ckpt (str):
            the pretrained tokenizer to load using
            `AutoTokenizer.from_pretrained`
        text_column (str):
            dataset feature column used as input text
        additional_inputs (dict[str, str]):
            specify additional inputs to the tokenizer's call method by mapping
            keyword argument names to dataset feature columns
        add_special_tokens (bool):
            whether to add specical tokens to the tokenized sequence
        padding (bool | str):
        truncation (bool | str):
    ...
    t: Literal[
        "hyped.data.processor.tokenizer"
    ] = "hyped.data.processor.tokenizer"
    ...
    padding: bool | Literal["max_length", "do_not_pad"] = False
...
class TokenizerProcessor(DataProcessor[TokenizerProcessorConfig]):
    ...
    def map_features(self, features: Features) -> Features:
        ...
        if self.config.text_column not in features:
            raise KeyError(
                "`%s` not present in features!" % self.config.text_column
            )
        ...
        if self.config.is_split_into_words and not (
            ...
            raise TypeError(
                ...
                % (self.config.text_column, f)
            )
        elif (not self.config.is_split_into_words) and (f != Value("string")):
            raise TypeError(
                "Input feature `%s` must be string, got %s."
                % (self.config.text_column, f)
            )
        ...
        for key, column in self.config.additional_inputs.items():
            if key not in sig.parameters.keys():
                raise TypeError(
                    "Tokenizer got unexpected additional keyword argument `%s`"
                    % key
                )
        ...
        if self.config.return_word_ids:
            new_features["word_ids"] = Sequence(
                Value(dtype="int32"), length=length
            )

        # special-cases
        if "boxes" in self.config.additional_inputs:
            new_features["bbox"] = Sequence(
                Sequence(Value(dtype="int32"), length=4), length=length
            )
